...
def simulationWithoutDrug(numViruses, maxPop, maxBirthProb, clearProb, numTrials):
    ...
    TIMESTEPS = 300
    avg_pop = [0 for _ in range(TIMESTEPS)]
    ...
    for trial in range(numTrials):
        ...
        for step in range(TIMESTEPS):
            avg_pop[step] += patient.update()

    avg_pop = [pop / numTrials for pop in avg_pop.copy()]
...
#     numViruses=100, maxPop=1000, maxBirthProb=0.1, clearProb=0.05, numTrials=100
# )
            ...
            if random.random() < (self.getMaxBirthProb() * (1 - popDensity)):
                ...
                for drug, resistance in self.getResistances().items():
                    if random.random() < (self.getMutProb()):
                        # Mutation happened, resistance is flipped
                        child_resistances[drug] = not resistance
                    else:
                        # No mutation
                        child_resistances[drug] = resistance
...
def simulationWithDrug(
    ...
    TIMESTEPS = 150
    avg_pop = [0 for _ in range(2 * TIMESTEPS)]
    avg_resist = [0 for _ in range(2 * TIMESTEPS)]

    for trial in range(numTrials):
        viruses = [
            ResistantVirus(
                maxBirthProb=maxBirthProb,
                clearProb=clearProb,
        ...
        for step in range(TIMESTEPS):
            avg_pop[step] += patient.update()
            avg_resist[step] += patient.getResistPop(["guttagonol"])
    ...
    avg_pop = [pop / numTrials for pop in avg_pop.copy()]
    avg_resist = [res / numTrials for res in avg_resist.copy()]

    pylab.plot(avg_pop, label="ResistantVirus")
    pylab.plot(avg_resist, label="Resistance to guttagonol")
