...
def r_squared(y, estimated):
    ...
    for sample, estimate in zip(y, estimated):
        squares_res.append((sample - estimate) ** 2)
        squares_tot.append((sample - mean) ** 2)

    sum_squares_res = sum(squares_res)
    sum_squares_tot = sum(squares_tot)
...
def evaluate_models_on_training(x, y, models):
    ...
    for model in models:
        ...
        estd_y = np.polyval(model, x)
        r_sq = r_squared(y, estd_y)
        pylab.plot(x, estd_y, "r", label=f"Fit")
        pylab.legend(loc="best")
        pylab.title(f"Fit of degree {len(model)}, R2 = {r_sq}")
        pylab.show()
